...
(This section describes the steps in the pipeline, detailed scripts are below)

1. Find an Orphan-Enriched RNA-Seq dataset:
	
	A) Search RNA-Seq datasets for your organism on NCBI, filter Runs (SRR) for Illumina, paired-end, HiSeq 2500 or newer.
	- Download Runs from NCBI (SRA-toolkit).  Only use runs with sufficient sequencing depth (this depends on estimated number of genes in the species, for Arabidopsis we use over XXXXX?@AS??? gbases)
	
	     1.A) If existing annotation is available, expression quantification is determined against every annotated gene using every SRR, with Kallisto.
	- Run phylostratR on current gene models to infer phylostrata of each gene model
	- Rank the SRRs according to highest number of expressed orphans and select feasible amounts of data to work with focusing on runs with the highest number of expressed orphans.
	
	     1.B) If no gene annotation is available, best practice is to maximize representation of all genes by including diverse conditions, and including  reproductive tissues and stresses in which orphan gene expression is high.

	B) If NCBI-SRA has no samples for your organism, and you are relying solely on RNA-Seq that you generate yourself, best practice is to maximize representation of all genes by including conditions like reproductive tissues and stresses in which orphan gene expression is high.

 _Pick one of the 2 _ab initio_ prediction tools below:_
...
6. Evaluate your predictions
   ...
   - Run `BUSCO` to see how well the conserved genes are represented in your final predictions
	 ...
	 - Run `phylostratR` to infer phylostrata, including infer orphan genes and other taxonomically-restricted genes in your predictions


7. NOT included as part of the orphan gene prediction pipeline, but a likely next step. Options for functional annotation and evidence of translation include: level of transcript, Ribo-Seq, protein MS/MS, homology,  `InterProScan`, and more.  Such results can be included as fields in the XXX@AS file, and will be useful to others. 



## Steps in detail, with scripts
All case studies used in the manuscript are listed in the **Table** [**here**](case-studies.md). Additional information can be found by clicking on each case study (methods, parameters and dataset used).  

## 1. Finding Orphan-Enriched RNAseq dataset from NCBI:

Go to [NCBI SRA](https://www.ncbi.nlm.nih.gov/sra) page and search with "SRA Advanced Search Builder". This allows you to build a query and select the Runs that satisfy your chosen requirements. For example:
...
Clicking the "Accession List" allows you to download all the SRR IDs in a text file format. For downloading data using the SRA Toolkit (use [`runSRAdownload.sh`](scripts/runSRAdownload.sh))
...
Note: depending on how much data you find, this can take a lot of time and resources (disk usage). You may need to select only a subset of total datasets. The best way to do this (as we were able to do in this workflow) is to screen SRRs based on number of previously-annotated orphan genes expressed in each SRR. 

(If you do NOT have a previous gene annotation, then select from SRA a group of hopefully 30-50 diverse SRRs (eg: stress response, flowering tissue, roots, pollen) and with very deep coverage. Alternately, depending on what is available in SRA, you may need to isolate and sequence your own. Then. skip the next few steps and move to "Run BRAKER" section.)
...
```
Once completed, merge the tsv files containing counts and TPM using [`joinr.sh`](scripts/joinr.sh):
...
### Run phylostratr to infer phylostrata of genes, and identify orphans and other taxonomically-restricted genes.

The input to phylostratr is the proteins predicted from your gene-predictions.
...
Note: phylostratr and a variety of options within it are described here https://doi.org/10.1093/bioinformatics/btz171. 


### Select diverse RNA-Seq data

Once the orphan (species-specific) genes are predicted, count the total number of orphan genes expressed (>1TPM) in each SRR, rank them based on % orphan expressed. Depending on how much computational resources you have, you can select the top X number of SRRs to use them as evidence for direct inference and as training data.

Note: for _Arabidopsis thaliana_, we used all of the SRRs that expressed more than 60% of the orphan genes (=38 SSRs)
...
## Run ab initio (BRAKER or MAKER) gene prediction software.
...
Once complete, run the second round of MAKER by modifying the following lines in `maker_opts.ctl` file:
...
Note: Cufflinks and Class2 may take a long time to process BAM files with deep depth regions. This can be speeded up if,instead of using a merged BAM file, use a single BAM file for each SRR sample in multiple transcript assembly programs to generate transcripts. If you plan to use more RNAseq data @AS???whywouldyou??, you can use the [`runHisat2.sh`](scripts/runHisat2.sh) and generate a single BAM file of mapped RNAseq reads.
...
Process the mapped reads to generate high confidence splice sites; these are useful for picking the most accurate transcripts. We use PortCullis [`runPortCullis.sh`](scripts/runPortCullis.sh) for that purpose:

```bash
runPortCullis.sh merged_SRA.bam
```

Create the input set of transcripts for Mikado.  Consolidate all the transcripts, removing redundancy, using Mikado prepare [`runMikado_round1.sh.sh`](scripts/runMikado_round1.sh).
...
This will generate a `DI_prepared.fasta` file that will be used for predicting ORFs using TransDecoder [`runTransDecoder2.sh`](scripts/runTransDecoder2.sh) as follows.
...
As a final step, pick the best transcripts for each locus based on XXXXXXX@AS @JL and annotate them as genes using Mikado pick.
...
## Completing the gene predictions
### 4A. MIND final step
...
Run Mikado using the same scripts as Step 3:
