...
- [output: DI_prepared.fasta.transdecoder.bed](#output-dipreparedfastatransdecoderbed)
	- [4A. BIND final step](#4b-bind-final-step)
	- [4B. MIND final step](#4a-mind-final-step)
...
## 1. Finding Orphan Enriched RNAseq datasets from NCBI:
...
Note: depending on how much data you find, this can take a lot of time as well as resources (disk usage). You may need to narrow down and select only a subset of total datasets. One way to do this is by selecting SRRs most likely to be diverse (eg: stress response, flowering tissue, or SRRs with very deep coverage). A better approach, which you can only use if you have existing gene annotations for your organism, is to screen them based on number of orphan genes expressed in each SRR. That is the approach we use here. Otherwise,  skip the next few steps and move to Section 2A "Run BRAKER" (or 2B "Run MAKER").
...
As a final step, we will pick best transcripts for each locus and annotate them as genes using Mikado pick.
